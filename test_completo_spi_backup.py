...
def send_subcmd(device, subcmd, data=None):
    ...
    report[1] = 0x00  # Packet counter (se puede incrementar)
    # Rumble data (neutral)
...
def get_battery_from_input(device):
    """Obtener batería del input report estándar (método rápido)"""
    # Cambiar a modo no-bloqueante temporalmente
    was_blocking = True  # Asumir que estaba en modo bloqueante
    device.set_nonblocking(True)
    ...
    # Esperar un poco para que lleguen datos frescos
    time.sleep(0.05)
    
    # Leer varios reportes para obtener batería
    found = False
    for attempt in range(50):  # Aumentar intentos
        data = device.read(64)
        
        if data and len(data) >= 3:
            report_id = data[0]
            
            # Aceptar cualquier tipo de input report
            if report_id in [0x21, 0x30, 0x31, 0x3F]:
                battery_byte = data[2]
                
                # Verificar que no sea 0x00 (dato inválido)
                if battery_byte != 0x00:
                    # Extraer nivel de batería usando fórmula DS4Windows para Joy-Con
...
for idx, joycon_info in enumerate(joycons):
    ...
    try:
        ...
        battery, charging = get_battery_voltage_spi(device)
        ...
        print("   ⏱️  Capturando paquetes durante 1 segundo...")
        
        # Despertar dispositivo antes de medir
        try:
            battery, charging = get_battery_from_input(device)
            time.sleep(0.05)
        except:
            pass
        
        polling_hz, polling_ms, packets = measure_polling_rate(device, duration_seconds=1.0)
...
for idx, ds4_info in enumerate(ds4_controllers):
    ...
    try:
        ...
        # 1. LEER BATERÍA
        print("🔋 Leyendo batería...")
        battery, charging = read_ds4_battery(device)
...
print("=" * 70)
print("\n✅ DATOS LEÍDOS:")
print("   • Joy-Con: Batería, Colores RGB, Serial, Firmware, Vibración")
print("   • DualShock 4: Batería, Vibración")
print("\n💡 CAPACIDADES:")
print("   • Lectura directa via SPI (Joy-Con)")
print("   • Lectura de HID reports (DS4)")
print("   • Control de vibración para ambos")
print("\n📊 DATOS DISPONIBLES PARA LA API:")
print("   1. Batería (%) + Estado de carga")
...
print("   6. Latencia y polling rate")
